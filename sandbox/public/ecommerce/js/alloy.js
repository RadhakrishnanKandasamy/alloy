...
*/
    var assignIf = function(target, source, predicate) {
      if (predicate()) {
        reactorObjectAssign(target, source);
      }
...
*/
    var createLogController = function(_ref) {
      ...
      return {
        ...
        createComponentLogger: function createComponentLogger(
          componentNamespace
        ) {
          return createLogger(
            console,
      ...
      return {
        ...
        sendRequest: function sendRequest(payload, endpointDomain) {
          ...
          var _options$expectsRespo = options.expectsResponse,
            expectsResponse =
              _options$expectsRespo === void 0 ? true : _options$expectsRespo,
...
*/
    ...
    var createOptIn = function(_ref) {
      ...
      var purposes = ALL;
      var cookieName = createOrgNamespacedCookieName(
        OPT_IN_COOKIE_NAME,
        config.orgId
      );
            ...
            if (warnings.length) {
              logger.warn(
                "Invalid event command options:\n\t - " +
                  warnings.join("\n\t - ") +
                  "\nFor documentation covering the event command see: " +
                  CONFIG_DOC_URI
              );
            }
...
*/
    ...
    var attachClickActivityCollector = function(
      config,
      eventManager,
      lifecycle
    ) {
...
*/
    var createExpected = function(message) {
      return function(isValid, key, value) {
        return isValid
          ? ""
          : "'" + key + "': Expected " + message + ", but got '" + value + "'.";
      };
    };
...
*/
    var chain = function(validator, other) {
      ...
      reactorObjectAssign(newValidator, validator, additionalMethods);
      return newValidator;
    };
...
*/
    ...
    var elementExpected = createExpected(
    ...
    var arrayOfValidator = function(elementValidator) {
      return function(key, currentValue) {
        return (
          ...
          currentValue.reduce(function(error, value, i) {
            var arrayKey = key + "[" + i + "]";
            return (
              error ||
...
*/
    ...
    var expected$9 = function expected(message) {
      ...
      reactorObjectAssign(newValidator, validator);
      return newValidator;
    };

    reactorObjectAssign(baseValidator, {
      expected: expected$9
    });
...
*/
    ...
    var getBrowser = memoize(function(window) {
      ...
      return matchUserAgent(
        ((_matchUserAgent = {}),
        _defineProperty(_matchUserAgent, EDGE, /Edge\/([0-9\._]+)/),
      ...
      return {
        getEcidFromLegacyCookies: function getEcidFromLegacyCookies() {
          ...
          var legacyEcidCookieValue =
            reactorCookie.get(secidCookieName) ||
            reactorCookie.get(amcvCookieName);
        ...
        createLegacyCookie: function createLegacyCookie(ecid) {
          var amcvCookieValue = reactorCookie.get(amcvCookieName);
...
*/
    ...
    var createComponent = function(
      ...
      var idMigrationEnabled = config.idMigrationEnabled,
        orgId = config.orgId;
      var identityCookieName = getNamespacedCookieName(
        orgId,
        IDENTITY_COOKIE_KEY
      );
      var deferredForEcid;
          ...
          onResponse: function onResponse(_ref3) {
            ...
            return optIn.whenOptedIn().then(function() {
              if (idMigrationEnabled) {
                var identityResultPayloads = response.getPayloadsByType(
                  "identity:result"
                );
                var ecidPayload = find(identityResultPayloads, function(
                  payload
                ) {
                  return (
                    payload.namespace &&
                    payload.namespace.code === ecidNamespace
                  );
                });
...
*/
    ...
    var processUrls = function processUrls(
...
*/
    ...
    var collectIfMatches = function collectIfMatches(
      collect,
      clickedElement,
      value
    ) {
...
governing permissions and limitations under the License.
*/
    var createComponent$1 = function(
      config,
      logger,
      availableContexts,
      requiredContexts
...
*/
    ...
    var createConfig = function createConfig(options) {
      return reactorObjectAssign(Object.create(null), options);
    };
    ...
    var createConfigValidator = function createConfigValidator(config) {
      ...
      var validator = {
        addValidators: function addValidators(newValidators) {
          return reactorObjectAssign(validators, newValidators);
        },
        validate: function validate() {
          var errors = Object.keys(validators).reduce(function(ac, key) {
            ...
            if (!isConfigValueProvided && hasDefault) {
              ...
              var errorMessage = configValidator.validate(
                key,
                configValue,
      ...
      return {
        ...
        sendEvent: function sendEvent(event) {
          ...
          var _options$isViewStart = options.isViewStart,
            isViewStart =
              _options$isViewStart === void 0 ? false : _options$isViewStart;
          ...
          return lifecycle
            ...
            .then(function() {
              var endpointDomain = payload.shouldUseIdThirdPartyDomain
            ...
            .then(function(response) {
              ...
              warnings.forEach(function(warning) {
                logger.warn(
                  "Warning received from server: [Code " +
                    warning.code +
                    "] " +
                    warning.message
                );
