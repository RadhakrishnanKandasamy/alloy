            ...
            for (var i = -thumbsToShow; i < thumbsToShow + 1; i++) {
              ...
              if (i < 0) {
                ...
                if (activeSlideIndex + i - 1 < 0) {
                  $(swiper.container)
                    .find(".swiper-button-prev .preview__img-" + Math.abs(i))
                    .css(
                      "background-image",
                      "url(" +
                        swiper.slides[slidesCount + i + 1].getAttribute(
                          "data-preview-bg"
                        ) +
                        ")"
                    );
                  ...
                  $(swiper.container)
                    ...
                    .css(
                      "background-image",
                      "url(" +
                        swiper.slides[activeSlideIndex + i].getAttribute(
                          "data-preview-bg"
                        ) +
                        ")"
                    );
                ...
                if (activeSlideIndex + i - 1 > slidesCount) {
                  $(swiper.container)
                    .find(".swiper-button-next .preview__img-" + i)
                    .css(
                      "background-image",
                      "url(" +
                        swiper.slides[i].getAttribute("data-preview-bg") +
                        ")"
                    );
                } else {
                  $(swiper.container)
        ...
        for (j = i; j >= -1; j--) {
          ...
          if (
            ...
            responsive[values[i]]["dotsEach"] =
              j < 0
                ? false
                : parseInt(c.attr("data" + aliaces[j] + "dots-each"));
          }
        }
      }
  ...
  if (plugins.rdNavbar.length) {
    plugins.rdNavbar.RDNavbar({
      stickUpClone: plugins.rdNavbar.attr("data-stick-up-clone")
        ...
        : false,
      stickUpOffset: plugins.rdNavbar.attr("data-stick-up-offset")
        ? plugins.rdNavbar.attr("data-stick-up-offset")
        : 1
    });
        ...
        callbacks: {
          onChangeStart: function(active, indexTo) {
            if (indexTo > step + 1) {
              ...
              for (
                var j = 0;
                j < this.$content.find(".rd-material-tab").length;
                j++
              ) {
                if (j <= step) {
                  var inputs = this.$content
                    .find(".rd-material-tab")
                    .eq(j)
